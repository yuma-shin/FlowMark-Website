...
export async function getFlowMarkDownloadLinks(): Promise<FlowMarkDownloadLinks> {
  ...
  cached = (async () => {
    try {
      ...
      const windowsSetupExe = findAsset(
        assets,
        [/(win|windows)/i, /setup/i, /\.exe$/i],
        releasePage,
      );
      const windowsPortableExe = findAsset(
        assets,
        [/(win|windows)/i, /portable/i, /\.exe$/i],
        releasePage,
      );
      const windowsZip = findAsset(
        assets,
        [/(win|windows)/i, /\.zip$/i],
        releasePage,
      );

      const macArmDmg = findAsset(
        assets,
        [/mac/i, /(arm|arm64|aarch64|silicon)/i, /\.dmg$/i],
        releasePage,
      );
      const macIntelDmg = findAsset(
        assets,
        [/mac/i, /(intel|x64)/i, /\.dmg$/i],
        releasePage,
      );
      const macUniversalZip = findAsset(
        assets,
        [/mac/i, /\.zip$/i],
        releasePage,
      );
      ...
      return {
        releasePage,
        windowsSetupExe:
          windowsSetupExe === releasePage ? windowsAnyExe : windowsSetupExe,
        windowsPortableExe,
        windowsZip,
